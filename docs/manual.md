...
- **Soft worker collision (interim).** New `applyUnitSeparation` step pass between unit advance and the trail-kill sweep. Per-tick gentle push (~1/4 of per-axis overlap each + small sim-RNG perturbation) on overlapping pairs; `UNIT_COLLISION_RADIUS = 0.20` tiles. Stationary defenders push other units but absorb no displacement themselves; workers in dump mode are exempt (the dump's whole point is "punch through enemies leaving a deadly trail"). Workers' `movingToNode → harvesting` transition widened to `HARVEST_AT_NODE_REACH_SQ ≈ 0.55²` so a worker bounced slightly off the node centre still picks up the harvest. Bumps `REPLAY_VERSION` 11 → 12; golden fixtures regenerated. **Status:** the axis-aligned-bounding-box, no-velocity model is at its limit — playtest read it as "still messy." Sub-phase **3.10.10** is queued for a proper velocity-based steering + collision rewrite; the current pass is the floor we ship until then.

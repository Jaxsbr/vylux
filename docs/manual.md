...
- **Worker collision (reverted).** A soft `applyUnitSeparation` step pass shipped briefly under 3.10.9 (axis-aligned per-tick push with sim-RNG perturbation), but three tuning iterations all read in playtest as worse than no collision at all — workers were either non-functional, jittery, or both. The pass was reverted on 2026-05-08 (`REPLAY_VERSION` 12 → 13; golden fixtures regenerated again). The widened `movingToNode → harvesting` transition (`HARVEST_AT_NODE_REACH_SQ ≈ 0.55²`) was kept — it's an independent improvement that lets a worker arriving slightly off the node centre still pick up the harvest. **Net effect for now:** workers can again visibly stack at the same position. Sub-phase **3.10.10** rebuilds movement on a velocity-based steering layer (per-unit `vx`/`vy` in `SimState`, accel/maxSpeed, friction, RNG kick on head-on collisions); collision will live there.

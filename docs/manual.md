...
> **Last sub-phase that touched these numbers:** 3.10.9 (game-feel pass v2: HUD redesign, resource visuals, soft worker collision, iso-rotated camera pan). Numeric tunings: `UNIT_COLLISION_RADIUS = 0.20` tiles, harvest reach widened to `0.55²`.
...
- **Soft worker collision.** New `applyUnitSeparation` step pass between unit advance and the trail-kill sweep. Per-tick gentle push (~1/4 of per-axis overlap each + small sim-RNG perturbation) on overlapping pairs; `UNIT_COLLISION_RADIUS = 0.20` tiles. Stationary defenders push other units but absorb no displacement themselves; workers in dump mode are exempt (the dump's whole point is "punch through enemies leaving a deadly trail"). Workers' `movingToNode → harvesting` transition widened to `HARVEST_AT_NODE_REACH_SQ ≈ 0.55²` so a worker bounced slightly off the node centre still picks up the harvest. Bumps `REPLAY_VERSION` 11 → 12; golden fixtures regenerated.
- **Iso-rotated camera pan.** The camera sits at `(+x, +y, +z)` and looks at origin, so its right vector in world space is `(+x, -z)` and its ground-projected up is `(+x, +z)` — both 45° off the world axes. Mouse drag and WASD/arrow keys now flow through `screenToWorldDelta` in `camera-controller.ts`, which rotates the screen-axis intent into world coords using that iso basis. Result: dragging right pans the map along the player's perceived horizontal (the bottom-left ↔ top-right diagonal in world coords); dragging down pans along the top-left ↔ bottom-right diagonal.

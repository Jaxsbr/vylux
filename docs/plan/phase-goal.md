...
#### Hover preview
- [x] Unit test (Vitest): hover logic given a hovered tile returns correct dim color for blue (`#0d4d57`) and red (`#5a2311`) — matches documented hexes in the Design direction section [US-04]
- [x] Playwright: entering blue placement mode and moving mouse over the grid sets `window.__vylux.state.hoveredTile` to a non-null `{ tileX, tileY }` [US-04]
- [x] Playwright: moving mouse from tile A to tile B causes tile A's material color to revert to `#0a0a0a` in the same frame tile B becomes the hovered tile [US-04]
- [x] Playwright: while hovering a tile in blue placement mode, the scene contains exactly one ghost mesh with opacity ~0.4 and faction-cyan emissive color (asserted via `window.__vylux.debug.ghost`) [US-04]
- [x] Playwright: switching from blue to red (`1` -> `2`) while hovering updates both hover tile color and ghost mesh color without a mouse move [US-04]
- [x] Playwright: moving mouse off the grid causes `window.__vylux.state.hoveredTile === null` and hides the ghost mesh (`window.__vylux.debug.ghost.visible === false`) [US-04]
- [x] Playwright: returning to `idle` (via `Escape`) hides ghost mesh and clears any hover highlight [US-04]
- [x] Playwright: hovering an occupied tile shows the dim faction highlight AND hides the ghost mesh (occupied signal) [US-04]
- [x] Playwright: rapid mouse sweep across the grid (20 `page.mouse.move` calls in < 200ms) never throws, ends with `hoveredTile` reflecting the final cursor position, and `window.__vylux.debug.ghostCount === 1` at the final frame (no leaked ghost meshes) [US-04]
- [x] Unit test (Vitest): `handlePointerMove` called 5 times synchronously with different coords leaves `state.hoveredTile` equal to the last coord only, and a `reconcileScene(state)` call after produces exactly one visible ghost mesh [US-04]

#### Click placement and occupancy
- [x] Unit test (Vitest): `tryPlace(state, tileX, tileY)` on an unoccupied tile returns `{ ok: true, state: nextState }` with `placedUnits.length` incremented by 1 and `mode: 'idle'` [US-05]
- [x] Unit test (Vitest): `tryPlace(state, tileX, tileY)` on an already-occupied tile returns `{ ok: false, reason: 'occupied' }` and state is unchanged [US-05]
- [x] Unit test (Vitest): `tryPlace` with out-of-bounds coords (`(-1, 0)`, `(0, 20)`, `(20, 20)`, `(NaN, 0)`) returns `{ ok: false, reason: 'out-of-bounds' }` and state is unchanged [US-05]
- [x] Playwright: after pressing `1`, hovering tile `(5, 5)`, and left-clicking: `placedUnits.length === 1`, entry has `{ tileX: 5, tileY: 5, type: 'blue' }`, mode is `idle`, ghost is hidden, cursor returns to default [US-05]
- [x] Playwright: same flow for red (press `2` then click `(7, 3)`): `placedUnits` has a `{ type: 'red' }` entry [US-05]
- [x] Playwright (per-faction): placed blue unit mesh renders with cyan emissive material; placed red unit mesh renders with red-orange emissive material (asserted via `window.__vylux.debug.placedMeshes[i].material.emissive`) [US-05]
- [x] Playwright: clicking the same occupied tile again in placement mode does not increase `placedUnits.length` and does not exit placement mode [US-05]
- [x] Playwright: left-click outside the grid while in placement mode exits to `idle` without adding to `placedUnits` [US-05]
...
- [x] Ghost mesh blue: `material.emissive.getHexString() === '00e5ff'` (documented cyan hex) [US-04]
- [x] Ghost mesh red: `material.emissive.getHexString() === 'ff5a1f'` (documented red-orange hex) [US-04]
- [x] Operator sign-off: ghost unit (blue) reads as a placeholder — translucent, cyan, positioned on the hovered tile [US-04]
- [x] Operator sign-off: ghost unit (red) reads as a placeholder — translucent, red-orange, positioned on the hovered tile [US-04]
- [x] Placed mesh blue: `material.transparent === false`, `material.opacity === 1.0`, `material.emissive.getHexString() === '00e5ff'` [US-05]
- [x] Placed mesh red: `material.transparent === false`, `material.opacity === 1.0`, `material.emissive.getHexString() === 'ff5a1f'` [US-05]
- [x] Operator sign-off: placed unit (blue) reads as committed — solid, emissive, visibly different from the ghost [US-05]
- [x] Operator sign-off: placed unit (red) reads as committed — solid, emissive, visibly different from the ghost [US-05]
- [x] Hover tile blue material: `material.color.getHexString() === '0d4d57'` (documented dim cyan) [US-04]
- [x] Hover tile red material: `material.color.getHexString() === '5a2311'` (documented dim red-orange) [US-04]

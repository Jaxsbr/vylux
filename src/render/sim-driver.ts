...
export type CommandsForTick = (match: Match) => Command[];
...
export function startSimDriver(
  ...
  function frame(now: number): void {
    ...
    // If we hit the cap, discard backlog so we don't spiral.
    if (stepsThisFrame === MAX_STEPS_PER_FRAME && now >= nextTickTime) {
